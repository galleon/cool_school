...
class TimeSlot(BaseModel):
    ...
    start_hour: float = Field(..., ge=0.0, le=24.0, description="Start time in 24-hour format (e.g., 9.0 = 9:00 AM)")
    end_hour: float = Field(..., ge=0.0, le=24.0, description="End time in 24-hour format (e.g., 17.0 = 5:00 PM)")
    ...
    def validate_quarter_hours(cls, v: float) -> float:
        ...
        if (v * 4) % 1 != 0:
            raise ValueError('Hours must be in quarter-hour increments (e.g., 9.0, 9.25, 9.5, 9.75)')
        return v
...
class Teacher(BaseModel):
    ...
    max_load_hours: float = Field(..., gt=0.0, le=40.0, description="Maximum teaching hours per week")
    qualified_courses: Set[str] = Field(default_factory=set, description="Set of course codes the teacher is qualified to teach")
    availability: List[TimeSlot] = Field(default_factory=list, description="Available time slots for teaching")
    ...
    def validate_course_codes(cls, v: Set[str]) -> Set[str]:
        ...
        for course_code in v:
            ...
            if not course_code.replace('-', '').replace('_', '').isalnum():
                raise ValueError(f'Course code "{course_code}" must be alphanumeric with optional hyphens/underscores')
        return v
        ...
        for i, slot1 in enumerate(v):
            for j, slot2 in enumerate(v[i + 1:], i + 1):
                if (slot1.day == slot2.day and 
                    not (slot1.end_hour <= slot2.start_hour or slot2.end_hour <= slot1.start_hour)):
...
class Room(BaseModel):
    ...
    features: Set[str] = Field(default_factory=set, description="Available room features (e.g., projector, computers)")
    ...
    def validate_features(cls, v: Set[str]) -> Set[str]:
        ...
        valid_features = {
            'projector', 'computers', 'whiteboard', 'smartboard', 
            'audio', 'video', 'lab_equipment', 'wheelchair_accessible'
        }
        for feature in v:
            if feature not in valid_features:
                raise ValueError(f'Unknown room feature: {feature}. Valid features: {", ".join(valid_features)}')
        return v
...
class CourseSection(BaseModel):
    ...
    timeslots: List[TimeSlot] = Field(..., min_items=1, description="Scheduled time slots for this section")
    enrollment: int = Field(..., ge=0, le=1000, description="Number of enrolled students")
    required_feature: Optional[str] = Field(None, description="Required room feature for this section")
...
class Assignment(BaseModel):
    ...
    teacher_id: Optional[str] = Field(None, description="ID of the assigned teacher (None if unassigned)")
    room_id: Optional[str] = Field(None, description="ID of the assigned room (None if unassigned)")
    assigned_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="When the assignment was made")
...
class TimelineEntry(BaseModel):
    ...
    entry: str = Field(..., min_length=1, max_length=500, description="Human-readable description of the event")
...
class ScheduleState(BaseModel):
    ...
    teachers: Dict[str, Teacher] = Field(default_factory=dict, description="All teachers indexed by ID")
    rooms: Dict[str, Room] = Field(default_factory=dict, description="All rooms indexed by ID") 
    sections: Dict[str, CourseSection] = Field(default_factory=dict, description="All course sections indexed by ID")
    assignments: Dict[str, Assignment] = Field(default_factory=dict, description="All assignments indexed by section ID")
    timeline: List[TimelineEntry] = Field(default_factory=list, description="Chronological log of scheduling events")
    ...
    def validate_assignment_references(self) -> 'ScheduleState':
        ...
        for section_id, assignment in self.assignments.items():
            ...
            if assignment.teacher_id and assignment.teacher_id not in self.teachers:
                raise ValueError(f'Assignment references non-existent teacher: {assignment.teacher_id}')
    ...
    def get_unassigned_sections(self) -> List[str]:
        ...
        return [
            section_id for section_id, assignment in self.assignments.items()
