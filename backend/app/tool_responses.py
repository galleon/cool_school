...
class LoadDistributionResponse(BaseModel):
    ...
    message: str = Field(..., description="Status message")
    histogram_path: Optional[str] = Field(None, description="Path to generated histogram image")
    loads: Dict[str, float] = Field(..., description="Teacher name to load mapping")
    statistics: Optional[Dict[str, float]] = Field(None, description="Load distribution statistics")
...
class ViolationInfo(BaseModel):
    ...
    max_load: Optional[float] = Field(None, ge=0.0, description="Maximum allowed load (for overload violations)")
...
class RebalancingResult(BaseModel):
    ...
    sections_added: List[str] = Field(default_factory=list, description="Sections assigned to this teacher")
    sections_removed: List[str] = Field(default_factory=list, description="Sections removed from this teacher")
...
class RebalancingResponse(BaseModel):
    ...
    message: str = Field(..., description="Status message")
    changes: List[RebalancingResult] = Field(default_factory=list, description="Changes made during rebalancing")
    statistics: Optional[Dict[str, float]] = Field(None, description="Load distribution statistics after rebalancing")
...
class SwapResponse(BaseModel):
    ...
    message: str = Field(..., description="Status message")
    result: Optional[SwapResult] = Field(None, description="Swap result details if successful")
...
class UnassignedResponse(BaseModel):
    ...
    message: str = Field(..., description="Status message")
    unassigned_sections: List[UnassignedSection] = Field(..., description="List of unassigned sections")
...
class AssignmentResponse(BaseModel):
    ...
    message: str = Field(..., description="Status message")
    result: Optional[AssignmentResult] = Field(None, description="Assignment result details if successful")
